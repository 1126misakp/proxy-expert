...
- **参考文档**：`references/server-configs.md`（三种服务端方案配置）、`references/client-config.md`（客户端 YAML 模板）、`references/troubleshooting.md`（故障排查）
...
读取 `references/server-configs.md`，根据用户选择的 PLAN（A/B/C）选对应模板，填入真实的密钥和上游信息，通过 SSH heredoc 写入 `/etc/sing-box/config.json`。
...
读取 `references/client-config.md`，将 `.proxy-keys.txt` 中的值填入模板，在当前目录生成 `clash-verge-config.yaml`。
...
**未通过：** 读取 `references/troubleshooting.md`，按层级排查。常见路径：
1. Clash 系统代理没开？→ 开启
2. 节点没选中？→ 代理页面选中节点
